...
// ---------------------------------------------------------------------------
// Deep merge: merges source into target recursively. Source values win.
// ---------------------------------------------------------------------------
...
const configPath = path.join(ROOT, "scorecard.json");
...
// ---------------------------------------------------------------------------
// Generate header row (game metadata fields)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Generate a half-inning section (away or home)
// ---------------------------------------------------------------------------
function generateHalfInning(config, side) {
  ...
  // Parse label — split on " / " for the styling (bold first part, light second)
...
// Page size helpers
// ---------------------------------------------------------------------------
// Named sizes in inches [width, height] (portrait orientation)
const PAGE_SIZES = {
  LETTER: [8.5, 11],
  A4: [8.27, 11.69],
  LEGAL: [8.5, 14],
...
// Resolve page config → { widthIn, heightIn, cssSize }
function resolvePageSize(page) {
  ...
  // Subtract margins (in inches, default 0.3)
  ...
  const availW = (widthIn - ml - mr) * 96; // px at 96dpi
...
// ---------------------------------------------------------------------------
// Calculate print zoom factor — each half-inning gets its own page
// ---------------------------------------------------------------------------
function calculatePrintZoom(config) {
  ...
  // Find the tallest single half-inning page
  let maxPageH = 0;

  for (const side of ['away', 'home']) {
    let height = 0;

    // Scorecard padding (reduced in print)
    height += 32;

    // Game header (appears on each page)
    ...
    // Footer: two items side by side, height = max
  ...
  // Get available page dimensions from config
  ...
  zoom = Math.min(zoom, 1); // never zoom in
  zoom = Math.floor(zoom * 100) / 100; // round down
...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* On screen: hide duplicate header/footer in second page */
